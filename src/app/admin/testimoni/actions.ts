...
export async function upsertTestimonial(testimonialData: Omit<Testimonial, 'created_at' | 'id'> & { id?: string }) {
    const supabase = createServiceRoleClient();
    
    const dataToUpsert = {
        ...testimonialData,
        id: testimonialData.id || crypto.randomUUID(),
    };

    const { data, error } = await supabase.from('testimonials').upsert(dataToUpsert, { onConflict: 'id' }).select().single();
    if (error) {
